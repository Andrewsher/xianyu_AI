            ...
            if np.random.randint(0, 2) == 0:
                current_img = opened_images[image_idx].crop(
                    (center_x - allowed_size // 2, center_y - allowed_size // 2,
                     center_x + allowed_size // 2, center_y + allowed_size // 2))
                current_img.save(os.path.join(args.target_dir, 'image', str(args.crop_size), 'origin', str(i) + '.png'))
                current_label = opened_labels[image_idx].crop(
                    (center_x - allowed_size // 2, center_y - allowed_size // 2,
                     center_x + allowed_size // 2, center_y + allowed_size // 2))
                current_label.point(lambda i: i * 64 - 1).save(os.path.join(args.target_dir, 'annotation', str(args.crop_size), 'origin', str(i) + '_label.png'))
            else:
                current_img = rotation_45_crop(opened_images[image_idx], center_x, center_y, allowed_size,
                                               allowed_size)
                current_img.save(os.path.join(args.target_dir, 'image', str(args.crop_size), 'rotate', str(i) + '.png'))
...
def parse_args():
    ...
    parser.add_argument('-l', '--target-dir', default='/home/amax/yzg/baseline-unet/random_crop', type=str)
    parser.add_argument('-s', '--crop-size', default=384, type=int)
