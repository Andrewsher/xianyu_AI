...
def _convND(ip, rank, channels):
    ...
    if rank == 3:
        x = Conv1D(channels, 1, padding='same', use_bias=False, kernel_initializer='he_normal', kernel_regularizer=l2(1e-5))(ip)
    elif rank == 4:
        x = Conv2D(channels, (1, 1), padding='same', use_bias=False, kernel_initializer='he_normal', kernel_regularizer=l2(1e-5))(ip)
    else:
        x = Conv3D(channels, (1, 1, 1), padding='same', use_bias=False, kernel_initializer='he_normal', kernel_regularizer=l2(1e-5))(ip)
    return x
...
def block_n(x):
    ...
    x = conv2d_bn_relu(x, filters=int(channel_num), kernel_size=(1, 1))
...
def create_model(input_shape=(224, 192, 1)):
...
if __name__ == '__main__':
    create_model(input_shape=(14, 12, 1024))
